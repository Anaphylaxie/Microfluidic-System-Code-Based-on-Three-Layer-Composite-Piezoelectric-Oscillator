...
class CompositeThermalConductivitySVMPredictor:
    ...
    def load_data(self, file_path, target_column="复合导热系数(W/m·K)"):
        """
        从Excel文件加载数据
    ...
    def save_prediction_to_excel(self, output_path=None):
        ...
        # 添加预测误差列
        df_result['绝对误差'] = np.abs(df_result['复合导热系数(W/m·K)'] - df_result['预测复合导热系数'])
...
if __name__ == "__main__":
    ...
    # 加载数据
    if predictor.load_data("1.19data(不含渗流阈值).xlsx"):
        # 数据预处理
