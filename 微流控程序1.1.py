...
# 强制设置中文字体
plt_fonts = [f.name for f in fm.fontManager.ttflist]
# 优先使用的中文字体列表
cn_font_list = ['SimHei', 'Microsoft YaHei', 'WenQuanYi Micro Hei',
                'Noto Sans CJK SC', 'Source Han Sans SC', 'Arial Unicode MS',
                'SimSun', 'NSimSun', 'FangSong', 'KaiTi']
cn_font = None
for font_name in cn_font_list:
    if font_name in plt_fonts:
        cn_font = font_name
        break
...
class HMIApplication:
    ...
    def _setup_fonts(self):
        """设置中文字体"""
        # 尝试使用系统默认中文字体
        font_options = ['Microsoft YaHei', 'SimHei', 'SimSun', 'NSimSun', 'FangSong', 'KaiTi']
        self.cn_font = None
        ...
        if not self.cn_font:
            ...
            self.cn_font = 'TkDefaultFont'
        else:
            # 设置全局默认字体
            default_font = tk.font.nametofont("TkDefaultFont")
            default_font.configure(family=self.cn_font, size=10)

            text_font = tk.font.nametofont("TkTextFont")
            text_font.configure(family=self.cn_font, size=10)

            fixed_font = tk.font.nametofont("TkFixedFont")
            fixed_font.configure(family=self.cn_font, size=10)
    ...
    def _create_menu(self):
        ...
        menubar.add_cascade(label="文件", menu=file_menu, font=(self.cn_font, 10))
        file_menu.add_command(label="导出CSV", command=self._export_csv, font=(self.cn_font, 10))
        file_menu.add_command(label="导出报表", command=self._generate_report, font=(self.cn_font, 10))
        file_menu.add_separator()
        file_menu.add_command(label="退出", command=self.root.quit, font=(self.cn_font, 10))

        # 视图菜单
        view_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="视图", menu=view_menu, font=(self.cn_font, 10))
        view_menu.add_command(label="时间窗口: 30秒", command=lambda: self._set_time_window(30),
                              font=(self.cn_font, 10))
        ...
        menubar.add_cascade(label="分析", menu=analysis_menu, font=(self.cn_font, 10))
        analysis_menu.add_command(label="频谱分析", command=self._show_spectrum, font=(self.cn_font, 10))
    ...
    def _create_dashboard(self):
        ...
        ttk.Label(status_frame, text="系统状态:", font=(self.cn_font, 12, 'bold')).pack(side=tk.LEFT, padx=5)
        self.status_label = ttk.Label(status_frame, text="待机", font=(self.cn_font, 12), foreground='gray')
        self.status_label.pack(side=tk.LEFT, padx=5)

        self.mode_label = ttk.Label(status_frame, text="[自动模式]", font=(self.cn_font, 10))
        self.mode_label.pack(side=tk.LEFT, padx=20)

        # 运行时间
        self.runtime_label = ttk.Label(status_frame, text="运行时间: 00:00:00", font=(self.cn_font, 10))
        ...
        for i, (key, name, unit, color) in enumerate(metrics):
            ...
            ttk.Label(frame, text=name, font=(self.cn_font, 10)).pack()
            self.metrics[key] = ttk.Label(frame, text="0.00", font=(self.cn_font, 16, 'bold'), foreground=color)
            self.metrics[key].pack()
            ttk.Label(frame, text=unit, font=(self.cn_font, 9)).pack()
    ...
    def _create_curve_panel(self):
        ...
        # 游标数据标签
        self.cursor_label = ttk.Label(curve_frame, text="游标: --", font=(self.cn_font, 9))
    ...
    def _create_control_panel(self):
        ...
        ttk.Label(mode_frame, text="运行模式:", font=(self.cn_font, 10)).pack(side=tk.LEFT)
        ...
        ttk.Label(param_frame, text="目标流量:", font=(self.cn_font, 10)).pack(side=tk.LEFT)
        ...
        ttk.Label(param_frame, text="mL/min", font=(self.cn_font, 10)).pack(side=tk.LEFT)
    ...
    def _create_alarm_panel(self):
        ...
        # 报警统计
        self.alarm_stats = ttk.Label(alarm_frame, text="活跃报警: 0 | 今日总计: 0", font=(self.cn_font, 9))
        self.alarm_stats.pack(side=tk.BOTTOM, anchor=tk.W, pady=2)

    def _create_status_bar(self):
        """创建状态栏"""
        self.status_bar = ttk.Label(self.root, text="就绪 | 系统未运行 | 数据帧: 0",
                                    relief=tk.SUNKEN, anchor=tk.W, font=(self.cn_font, 9))
